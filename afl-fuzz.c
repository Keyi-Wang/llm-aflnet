...
mqtt_packet_t packets[105];  // 最多存 105 个 CONNECT 报文
...
// 解析所有 MQTT 报文
size_t parse_msg(const uint8_t *buf, size_t buf_len, mqtt_packet_t *out_packets, size_t max_count) {
...
void print_mqtt_packet(const mqtt_packet_t *pkt, size_t index) {
...
// 可选：多轮调度以提高多样性
void dispatch_multiple_mutations(mqtt_packet_t *pkt, int num_packets, int rounds) {
...
int reassemble_single_msg(const mqtt_packet_t *pkt, u8 *output_buf, u32 *out_len) {
    ...
    // Prepare Variable Header + Payload
    u8 payload_buf[1024*1024];  // 临时缓冲区
    u32 payload_len = 0;
    ...
    switch (pkt->type) {
        case TYPE_CONNECT: {
            ...
            payload_len += write_remaining_length(payload_buf + payload_len, con->variable_header.property_len);
            memcpy(payload_buf + payload_len, con->variable_header.properties, con->variable_header.property_len);
            payload_len += con->variable_header.property_len;

            // Payload
            payload_len += write_utf8_str(payload_buf + payload_len, con->payload.client_id);

            // Will flag check
            if (con->variable_header.connect_flags & 0x04) {
                payload_len += write_remaining_length(payload_buf + payload_len, con->payload.will_property_len);
                memcpy(payload_buf + payload_len, con->payload.will_properties, con->payload.will_property_len);
                payload_len += con->payload.will_property_len;

                payload_len += write_utf8_str(payload_buf + payload_len, con->payload.will_topic);

                write_uint16(payload_buf + payload_len, con->payload.will_payload_len);
                payload_len += 2;
                memcpy(payload_buf + payload_len, con->payload.will_payload, con->payload.will_payload_len);
                payload_len += con->payload.will_payload_len;
            }

            // User name flag check
            if (con->variable_header.connect_flags & 0x80) {
                payload_len += write_utf8_str(payload_buf + payload_len, con->payload.user_name);
            }

            // Password flag check
            if (con->variable_header.connect_flags & 0x40) {
                write_uint16(payload_buf + payload_len, con->payload.password_len);
                payload_len += 2;
                memcpy(payload_buf + payload_len, con->payload.password, con->payload.password_len);
                payload_len += con->payload.password_len;
            }
            break;
        }
        ...
        case TYPE_SUBSCRIBE: {
            ...
            payload_len += write_remaining_length(payload_buf + payload_len, sub->variable_header.property_len);
            memcpy(payload_buf + payload_len, sub->variable_header.properties, sub->variable_header.property_len);
            payload_len += sub->variable_header.property_len;

            for (int i = 0; i < sub->payload.topic_count; ++i) {
                payload_len += write_utf8_str(payload_buf + payload_len, sub->payload.topic_filters[i].topic_filter);
                payload_buf[payload_len++] = sub->payload.topic_filters[i].qos;
            }
            break;
        }

        #define MAX_PUBLISH_BUF 1024*1024
        #define CHECK_SPACE(size) \
            do { \
                if (payload_len + (size) > MAX_PUBLISH_BUF) return -1; \
            } while (0)
        ...
        case TYPE_PUBLISH: {
            ...
            // 写入 Topic Name
            size_t topic_len = strlen(pub->variable_header.topic_name);
            CHECK_SPACE(2 + topic_len); // 2 字节长度前缀 + 内容
            payload_len += write_utf8_str(payload_buf + payload_len, pub->variable_header.topic_name);

            // 写入 Packet Identifier (if QoS > 0)
            if (pub->qos > 0) {
                CHECK_SPACE(2);
            ...
            // 写入 Property Length 和 Properties
            uint32_t prop_len = pub->variable_header.property_len;
            size_t prop_len_encoded = write_remaining_length(payload_buf + payload_len, prop_len);
            CHECK_SPACE(prop_len_encoded + prop_len);
            payload_len += prop_len_encoded;
            memcpy(payload_buf + payload_len, pub->variable_header.properties, prop_len);
            payload_len += prop_len;

            // 写入 Payload
            if (pub->payload.payload_len > 0) {
                CHECK_SPACE(pub->payload.payload_len);
                memcpy(payload_buf + payload_len, pub->payload.payload, pub->payload.payload_len);
                payload_len += pub->payload.payload_len;
            }
        ...
        case TYPE_UNSUBSCRIBE: {
            ...
            payload_len += write_remaining_length(payload_buf + payload_len, unsub->variable_header.property_len);
            memcpy(payload_buf + payload_len, unsub->variable_header.properties, unsub->variable_header.property_len);
            payload_len += unsub->variable_header.property_len;
        ...
        case TYPE_AUTH: {
            ...
            // Property Length + Properties
            payload_len += write_remaining_length(payload_buf + payload_len, auth->variable_header.property_len);
            memcpy(payload_buf + payload_len, auth->variable_header.properties, auth->variable_header.property_len);
            payload_len += auth->variable_header.property_len;
    ...
    switch (pkt->type) {
        ...
        case TYPE_PUBLISH: {
            first_byte = 0x30;
            const mqtt_publish_packet_t *pub = &pkt->publish;
            if (pub->dup)    first_byte |= 0x08;
            if (pub->qos)    first_byte |= (pub->qos << 1);
            if (pub->retain) first_byte |= 0x01;
            break;
        }
...
int reassemble_msg(const mqtt_packet_t *packets, u32 num_packets, u8 *output_buf, u32 *out_len) {
    u32 offset = 0;
    *out_len = 0;
    ...
    for (u32 j = 0; j < num_packets; ++j) {
        ...
        if (reassemble_single_msg(&packets[j], temp_buf, &temp_len) != 0) {
...
void fix_publish(mqtt_subscribe_packet_t *packets, int num_packets){
  //fix in order to make sure that the packets are valid
...
void fix(mqtt_packet_t *pkt, int num_packets) {
    if (pkt == NULL) return;
...
havoc_stage:
  // TODO: add parser + syntax aware fuzzing + fixer + reassembly
  // Step 1: Check if out_buf is valid according to parser(generated by LLM). If not, skip it (jump to havoc stage). Return structured messages (M2).

parser_through++;
pkt_num = parse_msg(out_buf, len, packets, 105);
...
if (!pkt_num) {
  ...
  // debug packets
  // printf("解析出的报文数量: %zu\n", pkt_num);
  for (size_t j = 0; j < pkt_num; j++) {
    // print_mqtt_packet(&packets[j], j);
  }
...
stage_max = 256; //Initially set to 256, can be adjusted based on performance
...
for(stage_cur = 0; stage_cur < stage_max; stage_cur++) {
  ...
  dispatch_multiple_mutations(packets, pkt_num, rounds);
  ...
  out_len = 0;
  // printf("pkt_num: %zu\n", pkt_num);
  if (reassemble_msg(packets, pkt_num, output_buf, &out_len) != 0) {
    continue;;
